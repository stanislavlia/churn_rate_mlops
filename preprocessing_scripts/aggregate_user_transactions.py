...
def aggregate_user_transactions(input_path, output_dir):
    ...
    aggregated_df = user_transactions_df.groupby('msno').agg({
        ...
        'transaction_date': 'max'  # Last transaction date
    })

    # Flatten the column multi-index
    aggregated_df.columns = ['_'.join(col).strip() if isinstance(col, tuple) else col for col in aggregated_df.columns.values]

    # Rename the last transaction date column for clarity
    aggregated_df.rename(columns={'transaction_date_max': 'last_transaction_date'}, inplace=True)

    # Reset the index
    aggregated_df.reset_index(inplace=True)

    # Convert the last transaction date to datetime if necessary
    aggregated_df['last_transaction_date'] = pd.to_datetime(aggregated_df['last_transaction_date'])
